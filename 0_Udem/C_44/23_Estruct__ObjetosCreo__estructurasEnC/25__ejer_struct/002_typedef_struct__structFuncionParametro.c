    ...
    switch(user_option){
      case 1:
        ...
        switch(alumno){
          ...
          case 3:
            system("clear");
            printf("\nSeleccionaste a %s: ", alumnoNuevo);
            printf("\nSus materias son: \n\t1) %s\n\t2) %s\n\t3) %s\n ",Nuevo.goro[0].Materia, Nuevo.goro[1].Materia, Nuevo.goro[2].Materia);
            break;
        ...
        switch(user_modificacion){
          case 1:
            ...
            for(int i=0; i<3; i++){
              ...
              fgets(Manolo.goro[i].Materia, 40, stdin); strtok(Manolo.goro[i].Materia, "\n");
            }
            for(int i=0; i<3; i++){
              printf("\n#=> %s",Manolo.goro[i].Materia);
            }
            break;
          case 2:
            printf("\n\t #=>> Manuela\n"); 
            for(int i=0; i<3; i++){
              printf("\nIngrese la materia %d: ",i+1);
              fgets(Manuela.goro[i].Materia, 40, stdin); strtok(Manuela.goro[i].Materia, "\n");
            }
            for(int i=0; i<3; i++){
              printf("\n#=> %s",Manuela.goro[i].Materia);
            }
            break;
          case 3:
            printf("\n\t #=>> %s\n", alumnoNuevo); 
